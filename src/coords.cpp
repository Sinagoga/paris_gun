...
CartesianCoordinates geod_to_cart(const GeodeticCoordinates geod_coords) {
    ...
    double n = R_eq/sqrt(1-E_2*sin_lat*sin_lat);

    CartesianCoordinates cart_coords;

    cart_coords.x = (n + geod_coords.altitude)*cos_lat*cos_lon;
    cart_coords.y = (n + geod_coords.altitude)*cos_lat*sin_lon;
    cart_coords.z = (((R_pol*R_pol)/(R_eq*R_eq))*n + geod_coords.altitude)*sin_lat;
